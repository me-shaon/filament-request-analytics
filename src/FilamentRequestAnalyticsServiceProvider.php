...
{
    public static string $name = 'filament-request-analytics';

    public static string $viewNamespace = 'filament-request-analytics';
        ...
        $package->name(static::$name)
            ->hasInstallCommand(function (InstallCommand $command) {
                $command
                    ...
                    ->publishMigrations()
                    ->askToRunMigrations()
                    ->askToStarRepoOnGitHub('me-shaon/filament-request-analytics');
            });
    ...
    public function packageBooted(): void
    {
        ...
        if (app()->runningInConsole()) {
            foreach (app(Filesystem::class)->files(__DIR__ . '/../stubs/') as $file) {
                $this->publishes([
                    $file->getRealPath() => base_path("stubs/filament-request-analytics/{$file->getFilename()}"),
                ], 'filament-request-analytics-stubs');
            }
        }

        // Testing
        Testable::mixin(new TestsFilamentRequestAnalytics);
    }

    protected function getAssetPackageName(): ?string
    {
        return 'me-shaon/filament-request-analytics';
    }
    ...
    {
        return [
            'create_filament-request-analytics_table',
        ];
    }
}
